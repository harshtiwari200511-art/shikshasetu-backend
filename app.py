...
def signup():
    ...
    if success:
        ...
        try:
            save_profile_to_db(email, default_profile)
        except Exception as e:
...
def login():
    ...
    if success:
        ...
        # Load persisted cognitive profile (if any) and merge
        try:
            persisted = load_profile_from_db(email)
            if persisted:
                profile.update(persisted)
        except Exception as e:
            print("LOAD PROFILE ERROR:", str(e))
...
def get_user():
    ...
    if "profile" not in session:
        ...
        # Merge persisted cognitive profile if present
        try:
            persisted = load_profile_from_db(email)
            if persisted:
                session["profile"].update(persisted)
        except Exception as e:
            print("LOAD PROFILE ERROR:", str(e))
...
def analyze_performance():
    ...
    try:
        ...
        if email and email != "default_user":
            ...
            try:
                save_profile_to_db(email, updated_profile)
            except Exception as e:
                print("SAVE PROFILE ERROR:", str(e))
    except Exception as e:
